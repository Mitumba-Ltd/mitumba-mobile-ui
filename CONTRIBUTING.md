...
- Preserve inherited pre-activation partial claims, interrupted decisions, durable merge attempts/outcomes, policy activation/supersession/root-v2 records, recovery-abort resolution authority uses/generations/lineage/supersessions/spends, and orphan refs as recovery evidence. Execute only the deterministic exact-state recovery algorithm in `docs/ISSUE_WORKFLOW.md`: atomically contend on its one fixed repository-wide mutex, validate every complete protocol commit envelope, and require exactly one fixed outcome per represented one-shot POST, decision-close PATCH, or normal-merge mutation. The live winner alone may record `pre-send-not-sent`; positive immutable termination tied to the original session-direct use alone permits a later expiry-indeterminate outcome. Both require no result/receipt/success/retry and end through a pre-incident or incident active-stop transaction that atomically releases the exact mutex; stale leases apply nothing. An unchanged aborted subject may restart only from the sole currently qualified lineage head whose exact human-authority use selects a closed safe option from the non-self-referential basis and existing eight dispositions; start atomically creates the same-mutex/start/spend/child-edge transaction. Immutable history is distinct from current qualification; stale or expired unspent generations are superseded, not marked spent. Completion suppresses the subject, while an abort requires distinct fresh durable evidence with a new stable evidence key. Never guess ownership, replay authority, retry an unknown/not-sent/expired request, or delete a changed ref. Exact ref retirement uses one atomic create/delete transaction with expected-absence and exact-old-OID leases, no plus refspec, and no branch update or permanent-tag deletion.
...
- A protected policy merge remains a candidate successor under its predecessor. Finish or abandon existing predecessor-bound work and start nothing new. Enumerate `policy-transition-pending` only under absent-mutex `preSelectionPolicyQuiescence`; after selection, require exact-bound-mutex/start `selectedPolicyTransitionQuiescence` through quarantine, action, and pre-transaction validation. One atomic terminal transaction creates successor activation, predecessor supersession, zero/one unchanged-root `root-activation:v2`, recovery result, and terminal while deleting the exact old mutex; rejection is a no-op that leaves the predecessor active. The terminal-only `policy-transition-completed` state never re-enters enumeration. No mutable current-policy ref/settings mutation, in-flight identity migration, or bootstrap replay exists.

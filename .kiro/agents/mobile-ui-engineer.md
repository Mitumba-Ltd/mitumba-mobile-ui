...
2. If the user authorized a queue run rather than one numbered issue, verify the earliest active release tracker's accepted fingerprint-bound activation, reconcile both repository WIP-slot refs against claims and PRs, then select the earliest-milestone issue carrying both `status:ready` and `agent:eligible`.
3. Recompute the issue-contract fingerprint defined in `docs/ISSUE_WORKFLOW.md` and match it to an accepted approval from the direct active user or a GitHub actor whose current repository permission verifies as `maintain` or `admin`. Confirm implementation dependencies are merged into the default branch, decision dependencies have verified decision-closure records, no `human-required` condition applies, and the issue contract is complete. Revalidate the issue and tracker after claim finalization and immediately before PR creation. If either fingerprint changed, apply the prescribed blocked transition and stop. Stop on any other ambiguity instead of silently filling product or architecture gaps.
...
- Claim one issue at a time with the atomic transaction in `docs/ISSUE_WORKFLOW.md`: post intent, derive the six-hour lease from GitHub `created_at`, acquire one fixed queue-slot ref and the issue-branch ref through Create a Reference with HTTP `201`, finalize labels/comments, then reread everything. Treat `422` as a lost race; never treat an identical existing ref or `git push` result as ownership. Do not begin local work before final verification.
...
- Fetch the atomically created `agent/issue-<number>-<slug>` ref at the recorded current-`main` SHA. Never stack work on an unmerged issue branch or create a competing local/remote branch.
- Implement exactly one component or engineering concern. Immediately before PR creation, revalidate both the issue approval fingerprint and active tracker activation fingerprint. Open exactly one PR containing `Closes #<number>` only if both still match, then retain that claim's WIP slot through review.
- When the PR is open, move the issue to `status:in-review`, remove `agent:claimed`, retain its WIP slot, and report the PR before selecting more work.
- Continue only when this session has no active implementation, a repository WIP slot is atomically available, and the next issue is independent and cannot invalidate an occupied slot's contract.
- Open a follow-up issue for discovered scope. Never fold it into the current PR for convenience.
- For `type:decision`, post the bounded proposal, move it to `status:needs-decision` plus `human-required`, release its slot after retiring the claim, and stop. Only after an accepted active-user or currently verified maintainer/admin approval bound to the current contract fingerprint may a resumed run create non-eligible atomic issues and update the tracker. Post and verify the prescribed decision-closure record before closing the decision without an implementation PR.
...
- On an expired claim or closed-unmerged PR, inspect the timeline, slot ref/head, branch/head, PR/head/state, claim base SHA, and current issue fingerprint first. Build the canonical incident fingerprint, require an accepted approval for exactly one disposition, and consume that approval once with the permanent `agent/recovery-consumed-<approval-comment-id>` tag ref. Requery the incident after HTTP `201`; stop on `422`, drift, ambiguity, or any attempt to replay the approval.
...
- Never merge an implementation PR as part of issue or queue execution. Only a later, separate user message that names both the reviewed PR and its exact head SHA can authorize a normal merge; if the SHA is omitted, ask for confirmation. Immediately before acting, revalidate issue/tracker fingerprints and recheck CI, trailers, mergeability, scope, and the exact head. Send that expected SHA with the merge request so a concurrent push fails, and require fresh authorization after any head change.
- When every implementation issue in a slice is merged and every decision issue has its verified decision-closure record, revalidate the active tracker's activation fingerprint and every required issue fingerprint before auditing its `type:release` tracker without treating it as an implementation queue item. Inspect the milestone, Changesets, generated release PR, public artifact, validation, CI, and deferred scope. Propose the semantic version and apply `status:release-ready` plus `human-required` only when every technical gate passes.

...
Every operator invocation starts from tracker #5, verifies its current fingerprint and independently computed program-map hash, reconciles the fixed `refs/heads/agent/recovery-active` mutex and both repository WIP slots, then verifies the single active release-or-discovery root before querying eligible children. Runtime expiry is a durable handoff for `standing-policy` and independently verifiable durable records, not a new authorization gate; a `session-direct-active-kiro-user` instruction instead expires with its positively identified runtime and cannot be reconstructed. The next invocation resumes the next safe phase from GitHub evidence under `docs/ISSUE_WORKFLOW.md`. A recovery-blocked #5 stops normal selection even when root and slot refs otherwise appear valid. With the mutex absent, the closed terminal-history classifier requires every permanent prior start to be fully completed or its abort uniquely suppressed by a later valid resolution-bound completion with exact authority, lineage, spend, option, and subject equality; unresolved/current/competing/unsuppressed/malformed starts and conflicting resolutions block, and historical refs remain visible in the base stage. An unchanged aborted business subject can proceed only through the sole currently qualified append-only lineage head: its exact human-authority use selects a closed safe option from a non-self-referential basis and the existing eight dispositions, and start atomically creates the same-mutex/start/spend/child-edge transaction. Stale or expired unspent generations are superseded rather than marked spent; completion suppresses the subject, while an abort requires distinct fresh durable evidence with a new stable evidence key. Each common one-shot POST has exactly one resource-observed, authenticated response-rejected, winner-only not-sent/request-indeterminate, or original-use-bound expiry-indeterminate outcome. Rejection carries exact response evidence and, like every terminal common outcome, permits no resource/result/receipt/effect/retry or later success adoption; pre-incident/incident abort releases the mutex atomically. Decision-close PATCH and normal merge retain their dedicated closed outcomes. A terminal decision operation derives closed `decision-stop:v1` and explicit `decision-stopped`; only evidence-preserving `abandon-decision` may consume it, while closure validation remains false. Policy-transition enumeration uses absent-current-mutex `preSelectionPolicyQuiescence` over the blocker-free visible history; selected execution uses exact-bound `selectedPolicyTransitionQuiescence` with the selected start plus byte-identical history, rejects changes/new starts, and successful all-or-nothing cutover yields terminal-only `policy-transition-completed`. Copied documentation never overrides live evidence.
...
6. execute that plan through the crash-safe, marker-keyed decision-expansion transaction in `docs/ISSUE_WORKFLOW.md`, using the v3 intent's exact base tree/date, predeclared canonical output-key digests, exactly one fixed outcome per terminalized one-shot POST (resource-observed, authenticated response-rejected with exact status/body bytes, winner-only pre-send-not-sent, request-indeterminate, or original-use-bound expiry-indeterminate), endpoint-truthful actor/app attribution, and canonicalizer/mode-bound receipts only for resource outcomes; normal and recovery execution share the same closed `issue-create`, `milestone-create`, or `issue-comment-append` endpoint/scope/resolved request, plan/wire hashes, operation ID, fixed attempt/outcome, and declared receipt. Rejection and every other terminal form have no resource/result/receipt/effect/retry and cannot later adopt success; an issue milestone resolves only from the uniquely named earlier validated milestone receipt, and session-direct resource outcome/receipt publication is atomic in the receiving runtime;
...
12. publish and validate the canonical decision record, expansion or strict no-output effects, closure comment, and fixed issue-close PATCH outcome, then atomically create `decision-closure-result:v1` with exact branch/slot retirement; only that final result passes `validateDecisionClosure` and closes the design issue without an implementation PR. Recovery resumes only the exact prefixed substates `decision-record-ready`, `decision-output-ready`, `decision-receipt-ready`, `decision-expansion-completion-ready`, `decision-closure-ready`, `decision-issue-close-ready`, and `decision-finalization-ready`, with byte-identical inner substate/transition and outer option substate; unprefixed aliases are invalid. A rejected/not-sent/indeterminate/expiry common record, output, or closure outcome—or a dedicated close terminal—derives `decision-stopped` and only non-closing abandonment.

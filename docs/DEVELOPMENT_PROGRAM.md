...
Each release tracker lists its child issues in its body checklist and links the decisions that shaped it. Tick a checklist row when its issue closes. Program tracker #5 lists the release and discovery roots; a new root is added there when a design issue creates it, or recorded as an explicit deferral.

Start each working session from tracker #5, confirm the active release root and its remaining children, then pick the next eligible child. GitHub is the live state, so reconstruct it with fresh queries rather than trusting a copied document or an earlier read. An interruption is a handoff, not a new approval gate: resume the next safe step from what the repository actually shows.
...
Messaging, confirmations and modal patterns, disputes and two-factor-authentication presentation, and VAZI experiences follow the core checkpoints. Each starts with discovery and becomes a numbered release only after its scope forms one coherent capability. Tracker #15 becomes the discovery root after `0.10.0` is released; completing it creates or defers later numbered slices but never produces a Changeset or a release PR itself, and no implementation issue or package change belongs to the discovery milestone.

Version numbers are planning targets. The operator derives the actual semantic version from the accumulated public changes, audits the generated release candidate, merges it normally, then verifies GitHub Actions OIDC publication as described in `docs/RELEASING.md`.
...
Dependencies are issue-level facts, not permission to stack branches. A dependent issue becomes `status:ready` only after every implementation blocker is merged and reachable from `main` and every decision blocker is closed with its choice recorded.
...
4. gather evidence, compare the viable alternatives, and select the strongest bounded outcome, recording the choice, the rejected options, and the consequences in the issue; escalate only for a [genuine escalation](ISSUE_WORKFLOW.md#genuine-escalation);
5. create one detailed issue per selected component or foundation concern, each starting at `status:needs-brief`;
6. add those issues to the governing tracker's checklist, and create a new milestone and slice tracker when the outcome needs its own release slice;
7. record every deferral on tracker #5 or the governing root so it stays visible;
8. close the design issue as completed once its choice and the resulting issues are recorded; a design issue never ships an implementation PR.
...
- every implementation issue is closed by its own independently reviewed, normally merged PR, and every decision issue is closed with its choice, alternatives, and consequences recorded;
...
- the release audit, the independent review of the exact head, the normal merge, GitHub Actions OIDC publication, and registry and provenance verification are each evidenced on the tracker.
